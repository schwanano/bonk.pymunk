...
class Rect:
    ...
    def rotation(self, dt):
        ...
        if self.rotating[0] and self.rotating[1]:
            def speed_set(body, gravity, damping, step_dt):
                to_body = Vector2(body.position - self.orb_center)
                current_dist = to_body.length()
                
                if current_dist > 0:
                    direction = to_body.normalize()
                    
                    # 1. Tangent Velocity
                    tangent_dir = direction.rotate(90)
                    tangent_vel = tangent_dir * self.vec_length * -self.orb_speed
                    
                    # 2. Radial Correction
                    if step_dt > 0:
                        error = current_dist - self.vec_length
                        radial_correction = -direction * (error / step_dt)
                        body.velocity = tuple(tangent_vel + radial_correction)
                    else:
                        body.velocity = tuple(tangent_vel)
                    
                    # 3. Synchronized Rotation (Injected into the physics step)
                    # Added the '-' to match your clockwise orbit direction
                    body.angular_velocity = self.rot_speed
                        
            self.body.velocity_func = speed_set
        else:
            # Standard standalone rotation if not orbiting
